...
> 日期：2026-09-08（面包板实测 2026-09-11）
> 状态：✅ 5 道手算题已完成，✅ Tinkercad 自动路灯已搭建并实测，✅ 面包板实物电路已搭建并实测
...
除 Tinkercad 仿真外，另在 830 孔面包板上按同一原理搭建了实物电路（光敏电阻 + PN2222 晶体管 + LED），接线与仿真一致：
...
- **暗光亮灯（LDR 阻值大 → 晶体管导通）:** [面包板电路-暗光亮灯.png](面包板电路-暗光亮灯.png)
- **白光亮灯（室内光下临界导通）:** [面包板电路-白光亮灯.png](面包板电路-白光亮灯.png)

> 实物现象与仿真一致：遮光后 LED 明显点亮；室内光照下 LDR 阻值仍在几 kΩ 量级，基极电压落在 0.5-0.8V 临界区，LED 微亮而不完全熄灭——这是 LDR 分压电路的固有特性，而非接线错误。

### 光敏控灯原理

核心链条：**光强 → LDR 阻值 → B 点电压 → 晶体管开关 → LED 亮灭**。

1. **LDR 随光变阻值**：半导体受光子照射产生自由电子（载流子），光强 → 载流子多 → 电阻小；光弱 → 载流子少 → 电阻大。
2. **分压决定 B 点电压**：
   ```
   Vb = 5V × R_LDR / (10kΩ + R_LDR)
   ```
   - 强光：LDR 小（几百 Ω~1kΩ）→ Vb ≈ 0.24V（低）
   - 暗光：LDR 大（几十 kΩ 以上）→ Vb ≈ 0.7~4V（高）
   LDR 是 B 点的下拉：阻值越小，越把 B 拉向 GND；阻值越大，10kΩ 上拉把 B 抬向 5V。
3. **B 点电压是开关门槛**：Vb < 0.7V → 晶体管截止 → Ic ≈ 0 → LED 灭；Vb > 0.7V → 饱和导通 → Ic ≈ 9~14mA → LED 亮。
4. **一句话**：光强 → 灯灭，光弱 → 灯亮。

> 实测 241mV（白天灭）、696mV（暗光/临界亮），正好卡在 0.7V 阈值两侧，验证了这条链条。

（完整仿真数据表与推导见仓库根目录 [README.md](../README.md) 的「任务二」小节。）
