...
车已脱离 USB 独立供电：6 V 电池盒并联两路，电机直连、只有开发板走降压，电池负极 / 降压 GND / TB6612 GND / ESP32 GND / HC-SR04 GND 五点共地。拔掉 USB 后串口监视器会断，属正常。
...
1. **五点共地**——电池负极、降压模块 GND、ESP32 GND、TB6612 GND、HC-SR04 GND 连一起。漏接的表现是"电机不动且测距恒为 0"。
...
**当前状态：** 未解决。补偿系数 `TRIM_RIGHT` 要实测量出来：车现在能脱离 USB 自由跑（电池经降压模块供电），但自由跑的时候就拿不到串口数据；插上 USB 读数据，线又拴住车，跑偏量里混满线缆干扰。等 Day 25 打通 Wi-Fi，用无线通道下发 trim、读回轨迹一次搞定。
