...
> 📌 **串口这条路是有窗口期的。** 现在车还插着 USB，所以能采；但 Day 24 降压模块到货后车要脱离 USB 独立跑（见 Day 21「成品化供电」），那时就没有串口了。Day 26 的"串口实时绘图"大概率要改成收 Wi-Fi 数据 —— 好在只需要把 `serial.Serial()` 换成 socket 读，本文件的解析与落 CSV 逻辑原样复用。
