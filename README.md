...
**表笔接 3V3（套上 1.0538 后）** — 读数 3.49V，过校正：

- 读数：[`day-14/表笔接入3V3修正后的读数.png`](./day-14/表笔接入3V3修正后的读数.png)
...
| 校准前读数 | 3.31 V |
| `CAL_SCALE` | `1.0055` |
| 校准后读数 | ~3.33 V（预期，待上传验证） |
| **误差** | **~0%（预期）** |

踩过的坑：一度把校准前读数误记成 3.16 V，算出 `1.0538`，上传后读数变成 **3.49 V（+4.8%）**，
比不校准的 −2.7% 还差。原始读数会在不同次测量之间漂移（当天两次：3.24 V → 3.31 V），
**系数必须用当次读数算**。详细数据与两点标定方案见 [`day-14/校准记录.md`](./day-14/校准记录.md)。
