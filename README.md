...
| [`day-01/`](./day-01/) … [`day-24/`](./day-24/) | 每日学习内容（截图、电路文件、代码与笔记） |
...
└── day-24/                      ← 第 24 天：README 写作与项目展示（ffmpeg 切 GIF + 手写 SVG 出电路图 + README 组织顺序）
```
...
**实拍**：[`五个电阻串联焊接.png`](./day-16/五个电阻串联焊接.png)（正面）、[`五个电阻串联背面焊接点.png`](./day-16/五个电阻串联背面焊接点.png)（背面焊点）、[`电烙铁和吸锡器拆焊_正面.png`](./day-16/电烙铁和吸锡器拆焊_正面.png)（拆焊·正面）、[`电烙铁和吸锡器拆焊_背面.png`](./day-16/电烙铁和吸锡器拆焊_背面.png)（拆焊·背面焊点）
...
✅ 已在 ESP32-S3 开发板上拆自己焊的元件：[`电烙铁和吸锡器拆焊_正面.png`](./day-16/电烙铁和吸锡器拆焊_正面.png)、[`电烙铁和吸锡器拆焊_背面.png`](./day-16/电烙铁和吸锡器拆焊_背面.png)
...
| 拆焊练习 | ✅ 已实战 | 拿 ESP32-S3 开发板拆自己焊的元件练的手；洞洞板上 5 个电阻留作成品记录，不拆 |
...
三条要点：**共地**（电池负极 / 降压模块 GND / ESP32 GND / TB6612 GND / HC-SR04 GND 五点相连，漏接表现为电机不动且测距恒为 0）；**VM 与 VCC 两路独立**（电机电流走电池，不经过开发板）；**电池盒装 4 节 = 6V**，理由见下节。
...
Day 24 已落地：6V 电池盒分两路，电机直连、只有 ESP32 走降压，车拔掉 USB 自己在瓷砖地上跑。接法：
...
6V 电池盒
   ...
   └──► MP1584EN IN ──► OUT 5V ──► ESP32「5V」引脚
                                    │
   电池负极 ──┬── 降压 GND ──┬── TB6612 GND ───────┴── ESP32 GND   （共地）
```

落地步骤里两条容易错的：

1. 6V 电池盒正负极各引出两路：一路进 TB6612 VM，一路进降压模块 IN
2. 降压 OUT 5V → ESP32「5V」引脚，负极 → GND，**共地**
3. 先不拔 USB，量完电压再拔；拔掉后串口监视器会断，属正常
...
标定要车**自由跑至少 2 米**。USB 线的问题不在供电而在拴着车：线一拽，跑偏量的判断里混满线缆干扰，量出来的补偿值没有意义。Day 24 接上 MP1584EN 之后车已经能脱离 USB 自己跑，但自由跑的时候拿不到串口数据；插上 USB 读数据，线又拴住了车。
...
```
Day 24  降压模块接好 → 车脱离 USB 自由跑（但没有数据通道）
...
> 📌 连带影响：车已经脱离 USB，Day 26 指南写的"串口实时绘图"没有串口可用 —— 实时绘图只能改成收 Wi-Fi 数据。Day 22 写的 `serial_logger.py` 解析与落 CSV 逻辑不用改，只需把 `serial.Serial()` 换成 socket 读。**ADC2（GPIO11-20）与 Wi-Fi 冲突，Day 25 起模拟量一律走 ADC1（GPIO1-10）。**
...
> 📌 **串口这条路有窗口期，而且窗口已经关了**：Day 24 接上 MP1584EN 降压模块之后车脱离 USB 独立跑，自由跑时没有串口；要采串口就得插 USB，插上线车又被拴住。Day 26 的实时绘图只能改成收 Wi-Fi 数据。解析与落 CSV 逻辑原样复用，只需把 `serial.Serial()` 换成 socket 读。
...
## 第 24 天 — README 写作与项目展示

> 日期：2026-09-24
> 状态：✅ 已完成
>
> 硬件：无新增（沿用 Day 21 的车：ESP32-S3 + TB6612FNG + HC-SR04 + 双 TT 马达 + 4×AA 电池盒）
> 核心：按「简介 / 硬件清单 / 电路图 / 代码说明 / 测试结果 / 踩坑与解决 / 下一步」给小车写一份能独立看的 README
> 项目 README：[`day-24/README.md`](./day-24/README.md)
> 演示：[`智能小车落地跑.gif`](./day-24/智能小车落地跑.gif)（地面实拍）｜[`超声波避障演示.gif`](./day-24/超声波避障演示.gif)（悬空验证）｜ 实物图：[`智能小车实物图.jpg`](./day-24/智能小车实物图.jpg) ｜ 接线图：[`智能小车接线图.png`](./day-24/智能小车接线图.png) ｜ 素材制作过程：[`素材制作过程.md`](./day-24/素材制作过程.md)

完整笔记：[`day-24/README.md`](./day-24/README.md)

### 目标

指南 Day 24 要求输出"完整 README 含 GIF 演示 + 电路图"，模板是：

```
# 项目名称 / ## 简介 / ## 硬件清单 / ## 电路图 / ## 代码说明 / ## 测试结果 / ## 踩坑与解决（问题1：xxx → 解决：yyy）/ ## 下一步
```

对照现实：README 格式这仓库早定了；**GIF 真没做过**（Day 21 存的是 26 秒 1080p MOV，30 MB）；Fritzing 库里既没有 ESP32-S3-WROOM-1 也没有手上这块 TB6612FNG 模块。

关键一点：**模板里的「踩坑与解决」指的是小车这个项目踩的坑，不是做素材过程中踩的坑。** 所以 [`day-24/README.md`](./day-24/README.md) 是一份项目 README，逐日过程笔记仍在 [`day-21/README.md`](./day-21/README.md)；做 GIF 和电路图的过程单独放在 [`素材制作过程.md`](./day-24/素材制作过程.md)。

### 简介

一辆两轮差速小车，靠前方一个 HC-SR04 自己躲障碍：直行时每 100 ms 测一次距离，近于 15 cm 就后退 → 转向 → 强制前进，然后恢复判断。障碍一直不挪开，它就一直绕，不会卡死。

![智能小车实物图](./day-24/智能小车实物图.jpg)

整车照：[`智能小车实物图.jpg`](./day-24/智能小车实物图.jpg)（1050×1400，346 KB）

![智能小车落地跑](./day-24/智能小车落地跑.gif)

地面实拍：[`智能小车落地跑.gif`](./day-24/智能小车落地跑.gif)（3 秒，440×782，6 fps，2.04 MB）——车放在瓷砖地上自己跑，靠近纸箱后后退 → 转向 → 强制前进
悬空验证（早期，轮子不着地）：[`超声波避障演示.gif`](./day-24/超声波避障演示.gif)（4 秒，440×248，8 fps，1.50 MB）
...
| DZQJ 2WD 亚克力双层底盘 | TT 马达 1:48 ×2、车轮 ×2、万向轮 ×2 | 1 | 车体 |
| TB6612FNG 双路 H 桥模块 | — | 1 | 驱动两路电机，带 STBY 使能脚 |
| HC-SR04 超声波模块 | 2 ~ 400 cm | 1 | 前方测距 |
| 电池盒 | 4 节 5 号 = 6 V | 1 | **分两路**：电机直连 TB6612 的 `VM`；另一路进降压模块 |
| MP1584EN 降压模块 | 5 V 固定输出，3 A | 1 | 把电池 6 V 降到 5 V 供开发板「5V」脚。**电机不经过它** |

车已脱离 USB 独立供电：6 V 电池盒并联两路，电机直连、只有开发板走降压，电池负极 / 降压 GND / TB6612 GND / ESP32 GND 共地。拔掉 USB 后串口监视器会断，属正常。

**刻意不上的元件**：MPU-6050（两轮 + 万向轮底盘不需要姿态就能跑）、SG90 舵机（Day 27 云台才上车）。少一组线就少一处故障点。

### 电路图

[`智能小车接线图.png`](./day-24/智能小车接线图.png)（1770×1410）｜源文件 [`智能小车接线图.svg`](./day-24/智能小车接线图.svg)
...
![智能小车接线图](./day-24/智能小车接线图.png)

三条不能错的：**五点共地**（电池负极 / 降压模块 GND / ESP32 GND / TB6612 GND / HC-SR04 GND，漏接表现为电机不动且测距恒为 0）；**`VM` 与 `VCC` 是两条独立的路**（电机电流走电池不经过开发板，并到同一电源上电机一转就把 3V3 拖垮）；**`STBY` 接 3V3 常高**（拉低则 TB6612 整体休眠）。
...
四个设计点：**迟滞**（单一阈值在 15 cm 临界点振荡，15 / 25 切出 10 cm 死区）；**`COMMIT` 是指南没有的**（转 40° 就恢复测距会贴墙抽搐，强制前进 300 ms 先离开障碍区）；**`pulseIn()` 超时压到 6 ms**（阻塞从 30 ms 降到 6 ms，超时结论"前方很远"跟远距一致，无信息损失）；**左右交替转向**（固定朝一边转会越转越深，交替相当于原地来回试探）。

duty 取 115 而不是 255：TT 马达额定 3 V，电池 6 V，靠占空比把**平均电压**压回 `115/255 × 6 ≈ 2.7 V`。判据是平均电压不是占空比数字；选 6 V 是为更低内阻、能供出两轮同时启动的峰值电流。
...
| 问题 | 原因 | 解决 |
|---|---|---|
| 只有左轮转，一阵子左一阵子右 | 3 V 电池内阻供不出两轮启动峰值电流 | 换 4 节 6 V，duty 压到 115 把平均电压拉回 2.7 V |
| `ledcRead()` 恒返回 0 | Core 3.x 回读不可靠 | 不回读，写进去的值自己记在变量里 |
| 车贴着障碍前后抽搐 | 单一阈值在 15 cm 临界点振荡 | 15 / 25 双阈值，切出 10 cm 迟滞死区 |
| 转完还是看到同一堵墙 | 转 40° 就恢复测距 | 加 `COMMIT` 强制前进 300 ms、期间不测距 |
| 串口打出来全是"后退 0 秒" | `%.0f` 把 0.5 四舍五入成 0 | 小于 1 秒的量直接打毫秒整数 |
| 车朝同一边越转越深 | 固定单一转向方向 | `turnLeftNext` 每轮翻边，左右交替 |
| 两个马达转速不同，直行跑偏 | TT 马达个体差异 | 未解决。`TRIM_RIGHT` 要实测量，等 Day 25 Wi-Fi 通了再下发 trim、读回轨迹 |

排查里最有价值的一步是**找只差一个变量的对照组**：单轮命令 `lf lb lz lc rf rb rz rc` 八条全过，而两轮命令 `1`-`8` 全不转，两组只差轮子数和占空比——于是排除 GPIO、接线、代码，直接锁定电源。先入为主认定是杜邦线接触不良的话，会一直在 6 根线上打转。
