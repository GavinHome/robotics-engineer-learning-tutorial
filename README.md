...
| [`day-01/`](./day-01/) … [`day-17/`](./day-17/) | 每日学习内容（截图、电路文件、代码与笔记） |
...
- **Day 13**：串口通信与调试（UART + Serial.printf + JSON 输出）
...
| 好/坏对比照片 | 延后补拍 | 先知道好坏差别再"故意焊坏"，放在练习末期更有针对性 |

### 下一步

- ⏳ 补拍**好/坏对比**照片（指南要求的产出，唯一未完成项）
- **Day 16**：通孔焊接进阶 —— 5 个电阻串到洞洞板、拆焊练习（吸锡器）
- 洞洞板到货后在新板上重做练习，新板铜箔干净，手感更顺
...
- 📌 **拆焊**：手法已记录在 `day-16/README.md` §八，**不单独占练习时间**，等真实场景（改孔位 / 回收元件 / Day 21 接线返工）直接照做
...
> 状态：✅ **已上电跑通并完成距离对照**（2026-09-19）：五档灯色全命中、空旷对照组通过、4 点误差 −0.1~−1.0cm
...
>
> ⚠️ **2026-09-19 接线方案修订**：VCC 由原先写的「5V」改为 **3V3**（见下方接线章节）
...
✅ **3V3 供电方案成立** —— 173cm 仍能稳定读出，不用退回「5V + 分压」方案（VCC 已确认接在 3V3 排针）。<br>
⚠️ **读数系统性偏小** —— 4 个点全为负（−0.1~−1.0cm），非随机跳动。主因是室温声速（~346m/s）快于代码用的 343m/s，理论偏移约 −0.87%；另有起算点差异。Day 21 只判有无障碍，1cm 内偏移无影响，**不做补偿，记录即可**。<br>
⏳ **待补**：同一距离 10 次连读的跳动范围、30cm 边界档、3~4m 远距点。
...
- ⏳ 同一距离连读 10 次，记下跳动范围（预计 ±0.3cm）
- ⏳ 30cm 边界档；3~4m 远距点（用来分离温度项与起算点偏移）
- **Day 18**：MPU-6050 IMU（I2C）—— ⚠️ 需提前装库 Adafruit MPU6050 + BusIO + Unified Sensor；模块已于 2026-09-19 到货
