...
三条不能错的：**四点共地**（电池负极 / ESP32 GND / TB6612 GND / HC-SR04 GND，漏接表现为电机不动且测距恒为 0）；**`VM` 与 `VCC` 是两条独立的路**（电机电流走电池不经过开发板，并到同一电源上电机一转就把 3V3 拖垮）；**`STBY` 接 3V3 常高**（拉低则 TB6612 整体休眠）。
...
> 📌 Day 24 原计划的"降压模块到货、车脱离 USB"没发生——MP1584EN 还在运输中（2026-09-22 下单）。到货当天：万用表量降压输出确认 5 V 再接 ESP32「5V」引脚；电机仍走电池 6 V 不经过降压；电池负极与开发板 GND 共地。
